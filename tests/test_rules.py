...
def test_first_flag_can_be_any_neutral_poi():
    ...
    res = engine.apply_move(state, "a", routing_service=FakeRoutingService())
    assert res.ok
...
def test_flipped_only_player_free_place_ok():
    """Player owns only flipped POIs (no anchor) → free place still allowed."""
    ...
    res_free = engine.apply_move(state, "c", routing_service=FakeRoutingService())
...
def test_second_flag_within_600s_valid():
    ...
    res1 = engine.apply_move(state, "a1", routing_service=FakeRoutingService())
    state = res1.state
    ...
    res2 = engine.apply_move(state, "p2", routing_service=FakeRoutingService())
    state = res2.state
    ...
    res3 = engine.apply_move(state, "c1", routing_service=routing)
    assert res3.ok
...
def test_cannot_flag_owned_poi():
    ...
    res = RulesEngine().apply_move(state, "a", routing_service=FakeRoutingService())
...
def test_cannot_flag_unknown_poi():
    ...
    res = RulesEngine().apply_move(state, "ghost", routing_service=FakeRoutingService())
    assert res.ok is False
...
def test_invalid_move_does_not_mutate_state():
    ...
    state = _state_with_pois(anchor, candidate)
    engine = RulesEngine()
    state = engine.apply_move(state, "a1", FakeRoutingService()).state  # P1 first
    p2 = make_poi("p2", east_m=5000)
    state.pois.append(p2)
    state = engine.apply_move(state, "p2", FakeRoutingService()).state  # P2 first
    ...
    res = engine.apply_move(state, "c1", routing_service=routing)
    assert res.ok is False
...
# ---------------------------------------------------------------------------
# Buffer geometry — 50m flips nearby, not far ones
# ---------------------------------------------------------------------------
...
def _p1_second_flag_setup(extras: list[Poi]):
    ...
    state = engine.apply_move(state, "a1", FakeRoutingService()).state
    p2 = make_poi("p2_first", east_m=5000)
    state.pois.append(p2)
    state = engine.apply_move(state, "p2_first", FakeRoutingService()).state
    return state, engine
...
def test_route_only_to_anchor_not_to_flipped_poi():
    ...
    routing = FakeRoutingService()
    routing.add(state.get_poi("c1"), state.get_poi("a1"), duration_s=250.0)
    state = engine.apply_move(state, "c1", routing).state
    ...
    state = engine.apply_move(state, "p2_second", FakeRoutingService()).state
    ...
    res = engine.apply_move(state, "c2", routing2)
...
def test_lost_anchor_does_not_appear_in_routes():
    ...
    res = engine.apply_move(state, "c1", routing)
    assert res.ok
...
def _state_with_two_p1_anchors():
    ...
    state = engine.apply_move(state, "p2f", FakeRoutingService()).state
    ...
    state = engine.apply_move(state, "a2", r_a2).state
    ...
    state = engine.apply_move(state, "p2s", r_p2s).state
...
def test_finished_after_max_turns():
    ...
    for i in range(12):
        ...
        res = engine.apply_move(state, target.id, routing)
...
def test_move_after_finished_is_invalid():
    ...
    res = RulesEngine().apply_move(state, "p", FakeRoutingService())
    assert res.ok is False
